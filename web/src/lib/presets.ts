...
export const LIMITS = {
  ...
  rattleFit: { min: 0.2, max: 3, step: 0.05 },
...
const ARCHETYPE_PRESETS: ShapePreset[] = [
  ...
  derive(
    ...
    'Meme famille que le jerkbait a bavette courte, mais le corps porte une texture de nervures transversales sur toute sa longueur : des anneaux reguliers perpendiculaires a l axe. Voir les nervures dans l onglet Scene.',
    {
      ...
      tailShape: 'taper',
      tailSize: 0.55,
      // Les nervures FONT cette famille : elles sont montees des le chargement.
      ribs: { ...defaultRibs(), enabled: true, pitch: 2, height: 0.4 },
    },
  ),
];
...
export const clonePreset = (id: ShapeId): LureParams => {
  ...
  return {
    ...
    rattles: params.rattles.map((item, i) => ({ ...item, id: `${item.id}-${i}-${Date.now()}` })),
