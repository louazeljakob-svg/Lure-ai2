  ...
  family(
    ...
    (base) => ({
      ...
      anatomy: {
        ...
        analFin: fin(0.79, 0.87, 0.07, 8),
        pectoralFin: fin(0.2, 0.28, 0.26, 11),
        pelvicFin: fin(0.38, 0.44, 0.16, 7),
        caudalRays: 16,
      },
      hasBib: false,
      tailShape: 'round',
      ...
      // Masses pesees de la quincaillerie du joint : deux oeillets inox et le
      // cylindre de retention imprime.
      articulation: articulation({
        enabled: true,
        segments: 2,
        positionMm: 84,
      ...
      assembly: assembly({
        pegs: { ...defaultPegs(), count: 4 },
        anchors: [
          anchor('nez', 0.06, 0, 'nose'),
          anchor('ventre', 0.3, -0.5, 'belly'),
          anchor('arriere', 0.72, -0.5, 'belly'),
        ],
      }),
      screws: {
        enabled: true,
        nutFit: 0.1,
        screws: [
          { id: 'vis-avant', position: 0.18, size: 'auto', head: 'countersunk', length: 25 },
          { id: 'vis-arriere', position: 0.655, size: 'auto', head: 'countersunk', length: 20 },
        ],
      },
      infill: 40,
      ballasts: [
