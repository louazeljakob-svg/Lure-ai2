...
const ui = {
  ...
  VwHeading: 180,      // MODULE F — angle du courant (deg) ; 180 = comportement scalaire d'origine
  rodHeight: 1.5,      // MODULE C — hauteur de la canne au-dessus de l'eau (m)
};
// MODULE E.2 — etat de nage global (chavirage)
const simState = { capsized: false, stabilityRatio: 99 };

// MODULES A & B : positions LOCALES (m) des composants, pour le bilan de masse, le
// centre de masse et la stabilite. Declare tot (avant updateParams) car updateParams
// lit geomComp.frameCentroid des le 1er appel. Renseigne a chaque buildLure.
const geomComp = {
...
const toggles = { streamlines: true, iso: false, pressure: false, hodo: true, stress: false, wake: true,
  vectors: false, bodyTrail: false, refAxis: false, axes: false }; // AXE 1.4 / 2.2 / 2.4 · MODULE A.1
...
function updateParams() {
  ...
  params.Vf = ui.Vf;
  params.Vw = ui.Vw;                                  // magnitude du courant (compat)
  // MODULE F — courant comme VRAI vecteur 2D horizontal (plan XZ). VwX est la
  // composante axiale, VwZ la composante laterale. Convention calee pour que
  // l'angle 180° (defaut) redonne EXACTEMENT le comportement scalaire d'origine :
  //   VwX(180°) = Vw·cos(180°) = -Vw  ->  Vrel_x = u + VwX = u - Vw   (identique).
  params.VwHeadRad = deg2rad(ui.VwHeading);
  params.VwX = ui.Vw * Math.cos(params.VwHeadRad);
  params.VwZ = ui.Vw * Math.sin(params.VwHeadRad);
  params.rodHeight = ui.rodHeight;
  ...
  /* MODULE E.1 — effet du POIDS AJOUTE (ballasts) sur la nage. Le ratio est
   * calcule par rapport a la masse SANS ballast, donc = 1 par defaut (aucun
   * ballast) -> aucun changement du comportement de reference. Physiquement
   * solide : plus de masse -> plus d'inertie -> oscillation plus lente et de
   * plus faible amplitude (baisse muBase/psiSatBase, coherent avec Iyaw du
   * module masse). Empirique/documente : au-dela d'un seuil, la nage devient
   * plus erratique (exces de poids -> bruit basse frequence renforce). */
  let massRatio = 1;
  if (massState.budget) {
    const bal = massState.budget.parComposant.ballasts || 0;
    massRatio = massState.massTotal / Math.max(massState.massTotal - bal, 1e-6);
  }
  params.massRatio = massRatio;
  params.muBase *= clamp(1 / (0.35 + 0.65 * massRatio), 0.45, 1.05);   // inertie -> amortit
  params.psiSatBase *= clamp(1 / (0.7 + 0.3 * massRatio), 0.55, 1.05);
  params.massExcess = clamp(massRatio - 1.6, 0, 3);                     // au-dela du seuil -> nervosite

  /* MODULE E.2 — critere de stabilite (chavirage). capaciteRedressement croit
   * avec l'ecart vertical CM/centre-de-carene (hauteur metacentrique : un CM
   * SOUS le centre de poussee redresse) ; couplePerturbateur croit avec la
   * portance de bavette et l'exces de poids. Seuils EMPIRIQUES documentes. */
  const dyRight = geomComp.frameCentroid.y - massState.com.y; // >0 : CM sous le centre de carene = stable
  const righting = 1.6 + 400 * dyRight;
  const perturb = 1.0 + 0.8 * Math.abs(Math.sin(params.lipAngleRad)) + 1.2 * Math.max(0, massRatio - 1);
  simState.stabilityRatio = clamp(righting, 0.01, 12) / perturb;
  // hysteresis pour eviter le clignotement autour du seuil
  if (simState.capsized && simState.stabilityRatio > 1.15) simState.capsized = false;
  else if (!simState.capsized && simState.stabilityRatio < 1.0) simState.capsized = true;
  params.capsized = simState.capsized;
  params.capsizeSide = (Math.sign(params.thetaEq) || 1); // sur quel flanc il bascule
}
updateParams();

// MODULE F — magnitude de la vitesse relative courante (addition vectorielle Vf/Vw),
// utilisee par l'affichage et les champs. A VwHeading=180° -> |u - Vw| (identique au
// scalaire d'origine). Vrel_x = u + VwX est la composante axiale.
function relSpeedAbs() { return Math.hypot(state.u + params.VwX, params.VwZ); }
// MODULE D — profondeur d'equilibre theorique (regime etabli du z-eq de derivatives) :
// zDotDot=0, zDot=0  ->  Fdive = kBuoy·z_eq  ->  z_eq = Fdive / kBuoy.  Modele
// simplifie (documente dans l'UI) : portance de bavette equilibree par la retenue
// verticale de ligne linearisee (kBuoy(rodHeight,lineLen)).
function computeZeq() {
  const VrelAbs = relSpeedAbs(), VrelSigned = state.u + params.VwX;
  const Fdive = 0.5 * RHO_WATER * params.ClDive * params.Slip * VrelAbs * VrelSigned * Math.sin(params.lipAngleRad);
  const rodFactor = clamp(1 + 0.18 * (params.rodHeight - 1.5), 0.4, 2.2);
  const kBuoy = params.kBuoyBase * (5 / Math.max(params.lineLen, 0.5)) * rodFactor;
  return clamp(Fdive / Math.max(kBuoy, 1e-6), 0, 3.5);
}
...
function derivatives(s, p) {
  // MODULE F — vitesse relative comme VRAI vecteur (addition Vf/Vw). L'ecoulement
  // relatif ressenti par le corps = vitesse d'avance u combinee au courant vectoriel :
  //   Vrel = ( u + VwX , 0 , VwZ )   [VwX,VwZ = courant projete, cf. updateParams]
  // Composante axiale (Vrel_x) -> trainee axiale (comme avant) ; magnitude ||Vrel||
  // -> tout ce qui depend de la vitesse (fShed, portance, chute...).
  // TEST DE COHERENCE (verifie) : a VwHeading=180° et |Vw|==Vf, en regime etabli
  // u->Vf, donc Vrel_x = u + Vw·cos(180°) = Vf - Vf = 0 : l'addition vectorielle
  // Vf_vec+Vw_vec s'annule bien a (0,0,0) -> aucune nage, comme attendu.
  const Vrel_x = s.u + p.VwX;
  const Vrel_z = p.VwZ;
  const VrelSigned = Vrel_x;                       // composante axiale (signe -> sens de la trainee)
  const VrelAbs = Math.hypot(Vrel_x, Vrel_z);
  ...
  // MODULE C — composante LONGITUDINALE de la tension de ligne (traction utile qui
  // avance le leurre). Le ressort 1D existant EST cette composante le long de l'axe
  // de retrieve ; les composantes verticale/laterale sont traitees separement
  // (retenue en profondeur via kBuoy(rodHeight/lineLen), derive laterale via VwZ).
  const Tline = p.kLine * s.xRel + p.cLine * (Vf - s.u);
  const Ddrag = 0.5 * RHO_WATER * p.Cd * p.Sfront * VrelSigned * Math.abs(VrelSigned);
  const uDot = (Tline - Ddrag) / p.mTrans;
  const xRelDot = Vf - s.u;

  // AXE 3.1 / MODULE E.1 — micro-irregularites de nage : modulation LENTE et FAIBLE
  // des coefficients qui pilotent psi (pas un forcage sur psi). Amplitude de base ~5%,
  // RENFORCEE par l'exces de poids (massExcess) -> nage plus erratique quand tres
  // lourd (effet empirique documente ; l'effet d'inertie, lui, est dans muBase/Iyaw).
  const exBoost = 1 + 0.9 * (p.massExcess || 0);
  const nFreq = 1 + 0.05 * exBoost * fbmNoise3(simTime * 0.15, 0, 0);
  const nAmp  = 1 + 0.045 * exBoost * fbmNoise3(simTime * 0.15 + 17.3, 0, 0);
  ...
  // MODULE E.2 — CHAVIRAGE : si le critere de stabilite est franchi, le cycle limite
  // s'eteint (mu -> negatif : amortit psi vers 0) et le roulis converge vers le flanc.
  const mu = (p.capsized ? -1.1 : p.muBase) * speedFactor;
  const psiDotDot = mu * omega0 * (1 - (s.psi * s.psi) / (psiThresh * psiThresh)) * s.psiDot - omega0 * omega0 * s.psi;

  // AXE 3.4 — asymetrie de bavette : position d'equilibre du roulis decalee (thetaEq).
  // MODULE E.2 — en chavirage, cible = flanc (~±80°) et couplage psi^2 coupe.
  const omegaTh = 2 * omega0, zetaTh = 0.35;
  const thetaTarget = p.capsized ? (p.capsizeSide * 1.4) : ((p.thetaEq || 0) * speedFactor);
  const thetaCouple = p.capsized ? 0 : p.gammaCouple * s.psi * s.psi * speedFactor;
  const thetaDotDot = -omegaTh * omegaTh * (s.theta - thetaTarget) - 2 * zetaTh * omegaTh * s.thetaDot + thetaCouple;

  // MODULE C/D — retenue verticale : la ligne (canne HAUTE au-dessus de l'eau) tire
  // le leurre vers la surface. Linearisee en un rappel kBuoy vers z=0, fonction de
  // la longueur deployee (ligne longue -> plus "a plat" -> retenue plus faible ->
  // plonge plus profond) ET de la hauteur de canne (canne haute -> retenue plus forte
  // -> remonte). A rodHeight=1.5 m (defaut) le facteur vaut 1 -> depth de reference.
  const rodFactor = 1 + 0.18 * (p.rodHeight - 1.5);
  const kBuoy = p.kBuoyBase * (5 / Math.max(p.lineLen, 0.5)) * clamp(rodFactor, 0.4, 2.2);
  const cZ = 1.8 * Math.sqrt(p.mTrans * kBuoy);
  // Portance de bavette : |Vrel|·Vrel_axial pour garder le signe du sens d'avance.
  ...
  // MODULE C/F — composante LATERALE : la nage (u·sin psi) + une derive due au courant
  // transversal (VwZ). A VwZ=0 (courant axial, defaut) -> comportement d'origine.
  const dispZDot = lateralVel + 0.5 * p.VwZ - 0.35 * s.dispZ;
...
function stepPhysics(dt) {
  ...
  const VrelEst = relSpeedAbs() + 0.05;
...
// MODULE A.1 — gizmo d'axes attache au leurre, a l'origine du repere local (= CG).
// AxesHelper : X rouge = nez (ecoulement entrant), Y vert = dos, Z bleu = flanc droit.
// Enfant de lureGroup (pas bodyGroup) -> non detruit par les rebuilds de geometrie.
const axesGizmo = new THREE.AxesHelper(1);
axesGizmo.visible = false;
lureGroup.add(axesGizmo);
...
// MODULE B.2 — points repositionnes manuellement (key -> Vector3 local memorise).
// Ils ne sont jamais ecrases par une detection automatique et sont exclus de
// l'inversion nez/queue (module A.2).
const charPointsManual = {};
...
function buildLure() {
  ...
  // MODULE B.2 — reapplique les positions manuelles par-dessus la detection auto
  for (const k in charPointsManual) if (charPoints[k]) charPoints[k].copy(charPointsManual[k]);
  buildCharMarkers();
}
...
function buildProceduralBody() {
  ...
  _geomOffset.set(0, 0, 0);
  // MODULE A.2 — inversion nez/queue disponible AUSSI sur le corps procedural :
  // le maillage est mis en miroir en X (scale -1 + DoubleSide) et toutes les
  // positions caracteristiques sont negees en X de façon coherente.
  const flip = geom.flip ? -1 : 1;
  ...
  bodyMesh = new THREE.Mesh(latheGeo, new THREE.MeshStandardMaterial({
    ...
    side: flip < 0 ? THREE.DoubleSide : THREE.FrontSide,
  }));
  bodyMesh.userData.baseColor = new THREE.Color(0x2f7ea8);
  bodyMesh.scale.x = flip;             // miroir du corps si flip
  bodyGroup.add(bodyMesh);

  const noseX = flip * (L - cgX), tailX = flip * (-cgX), lipLen = R * 2.1;
  ...
  charPoints.lipTip    = new THREE.Vector3(noseX - flip * lipLen * Math.cos(params.lipAngleRad), -R * 0.35 - lipLen * Math.sin(params.lipAngleRad), 0);
  ...
  charPoints.finLeft   = new THREE.Vector3(flip * (0.5 * L - cgX), R * 0.2, R * 1.05);
  charPoints.finRight  = new THREE.Vector3(flip * (0.5 * L - cgX), R * 0.2, -R * 1.05);
  charPoints.dorsal    = new THREE.Vector3(flip * (0.45 * L - cgX), R * 1.05, 0);
...
function processSTLGeometry(rawGeo, fileName) {
  ...
  // MODULE A.3 — indicateur de confiance du repere : ratio de variance axe1/axe2.
  // (la confiance definitive combine ce ratio avec le ratio d'ETENDUE de la bbox
  // le long des axes PCA, calcule plus bas — plus robuste sur un maillage dont
  // l'echantillonnage des sommets est irregulier, ex. sphere UV sur-echantillonnee
  // aux poles qui tromperait un ratio base uniquement sur la variance des sommets).
  const axisRatio = vals[order[0]] / Math.max(vals[order[1]], 1e-9);
  ...
  // MODULE A.3 — confiance finale : l'etendue de la bbox le long de l'axe principal
  // doit dominer nettement les axes secondaires (corps allonge = nez/queue evident).
  // Un mesh presque isotrope (sphere, cube -> ratio ~1) declenche l'avertissement.
  const extentRatio = (pMaxX - pMinX) / Math.max((pMaxY - pMinY), (pMaxZ - pMinZ), 1e-9);
  const axisConfident = axisRatio > 1.8 && extentRatio > 1.6;
  ...
  // (7) points caracteristiques (frame normalise)
  ...
  // MODULE B.1 — nez ROBUSTE multi-criteres : extremum sur X PUIS, parmi les ~5% de
  // sommets les plus proches en X, celui de plus PETITE section transversale (le plus
  // axial) = vraie pointe -> rejette un pic de triangulation isole hors-axe.
  function noseRobust(sign) {
    let ex = -Infinity;
    for (let i=0;i<proj.length;i+=3){ const v = sign*proj[i]; if (v>ex) ex=v; }
    let bi = -1, bestR = Infinity;
    for (let i=0;i<proj.length;i+=3){
      if (sign*proj[i] >= ex - 0.05) { const r = Math.hypot(proj[i+1], proj[i+2]); if (r<bestR){bestR=r;bi=i;} }
    }
    if (bi<0) bi = 0;
    return new THREE.Vector3(proj[bi], proj[bi+1], proj[bi+2]);
  }
  const nose = noseRobust(+1), tail = noseRobust(-1);
  const dorsal = extremeVertex(1, +1);
  const finLeft = extremeVertex(2, +1), finRight = extremeVertex(2, -1);

  // MODULE B.1 — anneau avant : cherche une protuberance VENTRALE nette juste
  // derriere le nez (point d'attache typique) ; sinon fallback estime (signale).
  let ringFront = null, ringEstimated = false;
  {
    let bi = -1, mostVentral = Infinity;
    const near = (x) => Math.abs(x - nose.x) < 0.22 && Math.abs(x) < Math.abs(nose.x) - 0.02;
    for (let i=0;i<proj.length;i+=3){
      if (near(proj[i]) && proj[i+1] < mostVentral) { mostVentral = proj[i+1]; bi = i; }
    }
    if (bi >= 0 && mostVentral < -(aspectY * 0.9)) ringFront = new THREE.Vector3(proj[bi], proj[bi+1], 0);
    else { ringFront = new THREE.Vector3(nose.x * 0.9, Math.min(nose.y, -0.06), 0); ringEstimated = true; }
  }
  const charNorm = {
    nose, tail, dorsal, finLeft, finRight, ringFront,
    ringRear:  new THREE.Vector3(tail.x * 0.95, 0, 0),
    lipTip:    new THREE.Vector3(nose.x + Math.sign(nose.x || 1) * 0.12, -aspectY - 0.1, 0),
  };
  ...
  geom.stl = {
    ...
    axisRatio, axisConfident, ringEstimated, // MODULE A.3 / B.1
  };
}
...
function fieldVelocityWorld(pWorld, out) {
  ...
  // MODULE F — flux uniforme = vecteur relatif (axial + lateral du courant).
  const VrelSigned = state.u + params.VwX;     // composante axiale
  const VrelAbs = relSpeedAbs() + 1e-4;
  const L = params.L, R = params.R;

  out.set(-VrelSigned, 0, -params.VwZ); // ecoulement uniforme relatif (axial + lateral), repere local
...
bindSlider('vwHeading', 'VwHeading', 'vwHeadVal', v => v.toFixed(0) + '°');       // MODULE F
bindSlider('rodHeight', 'rodHeight', 'rodHeightVal', v => v.toFixed(1) + ' m');   // MODULE C
...
function applyToggleVisibility() {
  ...
  // MODULE A.1 — repere local (taille ~0.6·L)
  axesGizmo.visible = toggles.axes;
  axesGizmo.scale.setScalar(0.6 * params.L);
}
applyToggleVisibility();
refreshFieldStatus();
...
function loadSTLFromBuffer(buffer, name) {
  ...
  setTimeout(() => {
    try {
      ...
      geom.mode = 'stl';
      buildLure(); resetHodo(); applyToggleVisibility(); updateReferenceLabel();
      ...
      // MODULE A.3 / B.1 — avertissements de fiabilite du repere
      let cls = 'ok';
      if (!st.axisConfident) { msg += ` · ⚠ axe principal peu marqué (ratio ${st.axisRatio.toFixed(1)}) — vérifiez/inversez le sens nez/queue`; cls = 'busy'; }
      if (st.ringEstimated) msg += ` · anneau avant : position estimée`;
      setStlStatus(msg, cls);
...
document.getElementById('stl-flip').addEventListener('click', () => {
  // MODULE A.2 — inversion nez/queue, coherente en mode STL ET procedural.
  // Si des points ont ete repositionnes manuellement, ils appartiennent a
  // l'ancien repere : on avertit et on les abandonne plutot que de les rendre
  // silencieusement incoherents (module B.2).
  if (Object.keys(charPointsManual).length > 0) {
    for (const k in charPointsManual) delete charPointsManual[k];
    setStlStatus('⚠ Points repositionnés réinitialisés par l\'inversion nez/queue.', 'busy');
  }
  geom.flip = !geom.flip;
  if (geom.mode === 'stl' && geom.lastBuffer) loadSTLFromBuffer(geom.lastBuffer, geom.lastName);
  else { updateParams(); buildLure(); resetHodo(); }
...
renderer.domElement.addEventListener('pointerup', (e) => {
  ...
  if (selectedKey && bodyMesh) {
    ...
    if (hb.length) {
      ...
      charPointsManual[selectedKey] = localP.clone(); // MODULE B.2 — memorise le point manuel
...
function updateMassPanelReadouts() {
  ...
  // MODULE E.2 — stabilite / chavirage affiche aussi dans le panneau masse
  const stab = simState.stabilityRatio;
  const stabTxt = simState.capsized
    ? `<span style="color:#ff7a6b">⚠ CHAVIRÉ — stabilité ${stab.toFixed(2)} (&lt; 1)</span>`
    : `stabilité <b style="color:${stab < 1.3 ? '#ff9a4d' : '#8effa0'}">${stab.toFixed(2)}</b>`;
  M.comCoords.innerHTML = `CM (mm) X=${(com.x*1000).toFixed(1)} Y=${(com.y*1000).toFixed(1)} Z=${(com.z*1000).toFixed(1)} · écart carène <b>${(com.distanceTo(car)*1000).toFixed(1)} mm</b><br>${stabTxt}`;
  drawFallCanvas(tv);
}
...
function updateLureTransform() {
  ...
  // MODULE C.1 — position REELLE du point A (bout de canne) en 3D. Hypotheses
  // simplificatrices (documentees) : canne fixe verticalement a `rodHeight` au-dessus
  // de l'eau (surface a y=0), pas de mouvement de l'angler. Le vecteur ligne
  // L_vec = posA - posAnneau se decompose en 3 composantes :
  //   - longitudinale  : le long de l'axe de retrieve (+X), la "traction utile"
  //   - verticale      : rodHeight + profondeur -> retient le leurre vers le HAUT
  //                      (ligne courte / canne haute -> composante plus grande ->
  //                       le leurre plonge moins profond ; cf. kBuoy(rodFactor))
  //   - laterale       : derive due au courant transversal / angle de casting
  // L'affichage est compresse a l'echelle de la scene (la vraie ligne fait plusieurs
  // metres) mais l'ANGLE reflete fidelement rodHeight/lineLen/profondeur.
  const ringWorld = charPoints.ringFront.clone().applyQuaternion(lureGroup.quaternion).add(lureGroup.position);
  const vertDrop = params.rodHeight + Math.max(state.z, 0);           // hauteur canne + profondeur
  const sinLine = clamp(vertDrop / Math.max(params.lineLen, 0.2), 0, 0.98); // inclinaison de la ligne
  const cosLine = Math.sqrt(Math.max(1 - sinLine * sinLine, 0));
  const latRatio = clamp(params.VwZ * 0.25, -0.4, 0.4);              // derive laterale (casting/courant)
  const dScene = 2.0 * L;                                            // longueur AFFICHEE (compressee)
  const dir = new THREE.Vector3(cosLine, sinLine, latRatio).normalize();
  const aPos = ringWorld.clone().addScaledVector(dir, dScene);
  aMarker.position.copy(aPos);
  // decomposition exposee (pour reference/rapport)
  params.lineLong = cosLine; params.lineVert = sinLine; params.lineLat = latRatio;
  const mid = ringWorld.clone().lerp(aPos, 0.5); mid.y -= 0.4 * L * clamp(1 - ui.lineK / 200, 0.15, 1);
...
/* MODULE A.1 — libelle permanent du repere, reflete l'etat courant de geom.flip. */
const refLabelEl = document.getElementById('refLabel');
function updateReferenceLabel() {
  if (!refLabelEl) return;
  const flipped = geom.flip;
  refLabelEl.innerHTML = 'Repère leurre : <b style="color:#ff6b6b">+X = nez</b> (écoulement) · ' +
    '<b style="color:#8effa0">+Y = dos</b> · <b style="color:#6b9bff">+Z = flanc droit</b>' +
    (flipped ? ' <span style="color:#ff9a4d">· nez/queue INVERSÉ</span>' : '');
}
updateReferenceLabel();

const capsizeBanner = document.getElementById('capsizeBanner');

/* MODULE F.2 — widget de decomposition vectorielle Vf / Vw (vue de dessus).
 * Montre Vf_vec (axial), Vw_vec (courant) et leur resultante Vf+Vw, avec les
 * composantes numeriques -> l'addition de vecteurs est explicite, pas deduite. */
const vecWidget = document.getElementById('vecWidget');
const vecCtx = vecWidget ? vecWidget.getContext('2d') : null;
function drawVectorWidget() {
  if (!vecCtx) return;
  const W = vecWidget.width, H = vecWidget.height, cx = W / 2, cy = H / 2;
  vecCtx.clearRect(0, 0, W, H); vecCtx.fillStyle = '#070a0e'; vecCtx.fillRect(0, 0, W, H);
  vecCtx.strokeStyle = '#1c2733'; vecCtx.lineWidth = 1;
  vecCtx.beginPath(); vecCtx.moveTo(cx, 0); vecCtx.lineTo(cx, H); vecCtx.moveTo(0, cy); vecCtx.lineTo(W, cy); vecCtx.stroke();
  // Vf_vec = (Vf,0) ; Vw_vec = (VwX,VwZ) ; resultante = somme. (X = axe retrieve horizontal)
  const scale = 40; // px par m/s
  const arrow = (vx, vz, col, lw) => {
    const ex = cx + vx * scale, ey = cy - vz * scale;
    vecCtx.strokeStyle = col; vecCtx.fillStyle = col; vecCtx.lineWidth = lw;
    vecCtx.beginPath(); vecCtx.moveTo(cx, cy); vecCtx.lineTo(ex, ey); vecCtx.stroke();
    const a = Math.atan2(ey - cy, ex - cx), hl = 6;
    vecCtx.beginPath(); vecCtx.moveTo(ex, ey);
    vecCtx.lineTo(ex - hl * Math.cos(a - 0.4), ey - hl * Math.sin(a - 0.4));
    vecCtx.lineTo(ex - hl * Math.cos(a + 0.4), ey - hl * Math.sin(a + 0.4));
    vecCtx.closePath(); vecCtx.fill();
  };
  const vfx = params.Vf, vwx = params.VwX, vwz = params.VwZ;
  arrow(vfx, 0, '#3fb6ff', 2);                 // Vf
  arrow(vwx, vwz, '#8effa0', 2);               // Vw
  arrow(vfx + vwx, vwz, '#ff9a4d', 2.4);       // resultante Vf+Vw
  vecCtx.font = '9px monospace';
  vecCtx.fillStyle = '#3fb6ff'; vecCtx.fillText(`Vf (${vfx.toFixed(2)}, 0)`, 5, 11);
  vecCtx.fillStyle = '#8effa0'; vecCtx.fillText(`Vw (${vwx.toFixed(2)}, ${vwz.toFixed(2)})`, 5, 22);
  vecCtx.fillStyle = '#ff9a4d'; vecCtx.fillText(`Σ (${(vfx+vwx).toFixed(2)}, ${vwz.toFixed(2)})  |V_rel|=${relSpeedAbs().toFixed(2)}`, 5, H - 6);
}
...
function animate() {
  ...
  const zeq = computeZeq(); // MODULE D.2 — profondeur d'equilibre theorique (modele simplifie)
  timeValEl.textContent = simTime.toFixed(2);
  regimeValEl.textContent = `V_rel = ${VrelAbs.toFixed(2)} m/s · f_shed = ${fShed.toFixed(2)} Hz · profondeur = ${state.z.toFixed(3)} m (z_eq≈${zeq.toFixed(3)} m, modèle simpl.) · Vf(t) = ${vfNow.toFixed(2)} m/s`;
  // MODULE E.2 — bandeau de chavirage
  if (capsizeBanner) capsizeBanner.style.display = simState.capsized ? 'block' : 'none';
  drawVectorWidget(); // MODULE F.2 — decomposition vectorielle Vf/Vw
